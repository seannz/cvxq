...
const int BLOCKHEIGHT =  24; // 256 * (3 / 32)
...
void vecquant3matmul_faster_cuda(
  ...
  int height = mat.size(0); // (12288 * 4) * (3 / 32)
  int width = mat.size(1); //  12288

  dim3 blocks(
    (height + BLOCKHEIGHT - 1) / BLOCKHEIGHT, // 12288 * 4 / 256 = 192
    (width + BLOCKWIDTH - 1) / BLOCKWIDTH     // 12288 / 256 = 48
  );
  dim3 threads(BLOCKWIDTH);
...
__global__ void VecQuant3MatMulKernel(
  ...
  int row = BLOCKHEIGHT * blockIdx.x; // 0, 24, 48, 96, ...
  int col =  BLOCKWIDTH * blockIdx.y + threadIdx.x; // 256 + (0, 1, ... 255), 512, ... 
